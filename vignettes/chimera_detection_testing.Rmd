...
- **`identify_contam_chimera_pq(method = "dada2")`**: dada2 de novo — same
  abundance-based logic, different algorithm.
...
tidypq's chimera detectors return a `contam_tbl`; the benchmarks below wrap
them with `filter_contam_pq()` so each method yields a chimera-free phyloseq
object.
