...
- **`identify_contam_chimera_pq(method = "dada2")`**: dada2 de novo —
  same abundance-based logic, different algorithm.
- **`identify_contam_chimera_pq(method = "vsearch_ref")`**: vsearch
  reference-based (UCHIME_ref) — compares each query against a trusted
  reference database, independent of sample composition.

tidypq’s chimera detectors return a `contam_tbl`; the benchmarks below
wrap them with
[`filter_contam_pq()`](https://adrientaudiere.github.io/tidypq/reference/filter_contam_pq.md)
so each method yields a chimera-free phyloseq object.
...
# Mini UNITE database bundled with MiscMetabar (used for remove_chimera_vs_ref)
...
remove_chimera_dada2 <- function(physeq, ...) {
  filter_contam_pq(
    physeq,
    identify_contam_chimera_pq(physeq, method = "dada2", verbose = FALSE),
    verbose = FALSE
  )
}
remove_chimera_vs_ref <- function(physeq, database, ...) {
  filter_contam_pq(
    physeq,
...
for (mult in abundance_multipliers) {
  ...
  start_time <- Sys.time()
  nochim_dada2 <- remove_chimera_dada2(result_abund$physeq)
  time_dada2 <- as.numeric(difftime(Sys.time(), start_time, units = "secs"))
  rate_dada2 <- calc_detection_rate(nochim_dada2, result_abund$chimera_names)

  start_time <- Sys.time()
  nochim_vs_ref <- remove_chimera_vs_ref(result_abund$physeq, database = mini_db)
  time_vs_ref <- as.numeric(difftime(Sys.time(), start_time, units = "secs"))
...
for (min_dist in c(0, 0.001, 0.01, 0.05, 0.1, 0.2, 0.4)) {
  ...
  nochim_dada2 <- remove_chimera_dada2(result_dist$physeq)
  rate_dada2 <- calc_detection_rate(nochim_dada2, result_dist$chimera_names)

  nochim_vs_ref <- remove_chimera_vs_ref(result_dist$physeq, database = mini_db)
