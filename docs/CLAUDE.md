...
**tidypq** is an R package providing tidyverse-style verbs for
manipulating phyloseq objects at four scales: samples, taxa,
occurrences, and tree. All functions follow the naming convention
`{verb}_{scale}_pq()`.
...
Functions use rlang data masking with a special `.` pronoun that refers
